...
from urllib.request import Request, HTTPRedirectHandler, ProxyHandler
from urllib.request import HTTPPasswordMgrWithDefaultRealm, HTTPBasicAuthHandler
from urllib.request import getproxies, build_opener, install_opener
...
class Request(Request):
    """Subclass of :class:`Request` that supports custom methods."""
...
class Response(object):
    ...
    def __init__(self, request, stream=False):
        """Call `request` with :mod:`` and process results.
        ...
        # Execute query
        try:
            self.raw = request.urlopen(request)
        except HTTPError as err:
            self.error = err
            try:
                self.url = err.geturl()
            # sometimes (e.g. when authentication fails)
            # can't get a URL from an HTTPError
    ...
    def raise_for_status(self):
        ...
        error will be instance of :class:`HTTPError`
        """
...
def request(method, url, params=None, data=None, headers=None, cookies=None,
    ...
    * ``mimetype`` is optional. If not provided, :mod:`mimetypes` will
    ...
    # Default handlers
    openers = [ProxyHandler(getproxies())]
    ...
    if files:
        ...
        data = parse.urlencode(str_dict(data))
    ...
    if params:  # GET args (POST args are handled in encode_multipart_formdata)
        ...
        if query:  # Combine query string and `params`
            url_params = parse.parse_qs(query)
            # `params` take precedence over URL query string
            url_params.update(params)
            params = url_params

        query = parse.urlencode(str_dict(params), doseq=True)
